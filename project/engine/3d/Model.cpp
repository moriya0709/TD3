...
void Model::Draw(const SkinCluster& skinCluster)
{
	...
	// （※ObjectCommonで追加した [10] 番のパラメータに、パレットのSRVを渡します）
...
ModelData Model::LoadModelFile(const std::string& directoryPath, const std::string& filename) {
	...
	for (uint32_t materialIndex = 0; materialIndex < scene->mNumMaterials; ++materialIndex)
	{
		...
		// マテリアルから自己発光カラーを取得できたら上書きする
		if (material->Get(AI_MATKEY_COLOR_EMISSIVE, emissiveColor) == aiReturn_SUCCESS) {
			modelData.material.emissive = { emissiveColor.r, emissiveColor.g, emissiveColor.b };
		} else {
			// 取得できなかった場合は確実に黒を入れる
			modelData.material.emissive = { 0.0f, 0.0f, 0.0f };
		}
	}
...
void Model::CreateVertexData()
{
	...
	vertexResource = dxCommon_->CreateBufferResource(sizeInBytes);
