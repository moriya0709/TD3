...
void ObjectCommon::CreateRootSignature() {
	...
	skinningRange[0].BaseShaderRegister = 1; // t1レジスタ
	...
	rootParameters[10].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // 頂点シェーダーで使う
	rootParameters[10].DescriptorTable.pDescriptorRanges = skinningRange;
	rootParameters[10].DescriptorTable.NumDescriptorRanges = _countof(skinningRange);
...
void ObjectCommon::CreateSkinningGraphicsPipeline()
{
	...
	// 生成
	hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&skinningPipelineState));
	assert(SUCCEEDED(hr));
}

// 描画時に呼び出すセット関数
void ObjectCommon::SetSkinningPipelineState() {
	dxCommon_->GetCommandList()->SetGraphicsRootSignature(rootSignature.Get());
	dxCommon_->GetCommandList()->SetPipelineState(skinningPipelineState.Get());
	dxCommon_->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}