#include "rushEnemyBullet.h"

void rushEnemyBullet::Initialize(Camera* camera, Vector3 Pos)
{
    camera_ = camera;
    ...
    object_->SetModel("itugoBullet.obj");
...
void rushEnemyBullet::Draw3D()
{
    // 3Dオブジェクト描画
    object_->Draw();
}

void rushEnemyBullet::OnCollision()
{
    isAvile = false;
}

void rushEnemyBullet::SetTargetPosition(Vector3 Pos)
{
    // 狙う場所を設定
    targetPos_ = Pos;
